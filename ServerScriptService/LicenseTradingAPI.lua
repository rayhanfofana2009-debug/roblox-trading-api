			...
			local Secrets = require(ServerStorage:WaitForChild("Secrets"))
			...
			local function makeRequest(method, endpoint, data, headers, validator, timeout, skipRetryOnTimeout)
				...
				if requestData.licenseId then
					local newUrl = url:gsub(":licenseId", HttpService:UrlEncode(tostring(requestData.licenseId)))
					if newUrl ~= url then
						url = newUrl
				...
				if requestData.userId then
					local newUrl = url:gsub(":userId", HttpService:UrlEncode(tostring(requestData.userId)))
					if newUrl ~= url then
						url = newUrl
			...
			local function getLicenseDetails(licenseIds)
				local licenses = {}

				for _, id in ipairs(licenseIds) do
					table.insert(licenses, {
						id = id,
						name = "License " .. string.sub(id, 1, 8),
					})
				end
			...
			local function buildUpdateForPlayer(trade, playerUserId)
				...
				return {
					tradeId = trade.id,
					myOfferedLicenses = getLicenseDetails(isPlayerA and trade.playerAOffer or trade.playerBOffer),
					theirOfferedLicenses = getLicenseDetails(isPlayerA and trade.playerBOffer or trade.playerAOffer),
				...
				for _, transfer in ipairs(result.data.data.transferredLicenses) do
					if transfer.toUserId == tostring(trade.playerA) then
						table.insert(receivedByA, {
							...
							name = "License " .. string.sub(transfer.licenseId, 1, 8),
						})
			...
			GetPlayerLicensesRE.OnServerEvent:Connect(function(player, requestId)
				...
				if result.success and result.data and result.data.data then
					...
					for _, license in ipairs(result.data.data) do
						table.insert(transformedLicenses, transformLicenseForClient(license))
					end
